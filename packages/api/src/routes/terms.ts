...
import { terms, withTenantContext } from '@monorepo/db';
...
const listQuery = z.object({ academicYearId: z.string().uuid().optional() });
...
const termsRoutes = new Hono<TenantEnv>()
  .get('/', zValidator('query', listQuery), async (c) => {
    ...
    const rows = await withTenantContext(appDb, { schoolId, userId }, (tx) =>
      tx
        ...
        .where(
          academicYearId
            ? and(eq(terms.schoolId, schoolId), eq(terms.academicYearId, academicYearId))
  ...
  .get('/:id', zValidator('param', idParam), async (c) => {
    ...
    const [row] = await withTenantContext(appDb, { schoolId, userId }, (tx) =>
      tx.select().from(terms).where(and(eq(terms.id, id), eq(terms.schoolId, schoolId)))
    );
  ...
  .post('/', zValidator('json', createSchema), async (c) => {
    ...
    try {
      const [row] = await withTenantContext(appDb, { schoolId: tenant.schoolId, userId: tenant.userId }, (tx) =>
        tx.insert(terms).values({ ...body, schoolId: tenant.schoolId }).returning()
      );
    ...
    try {
      const [row] = await withTenantContext(appDb, { schoolId: tenant.schoolId, userId: tenant.userId }, (tx) =>
        tx
          .update(terms)
          .set(body)
          .where(and(eq(terms.id, id), eq(terms.schoolId, tenant.schoolId)))
          .returning()
      );
      if (!row) return c.json({ error: 'Not found' }, 404);
      return c.json(row);
